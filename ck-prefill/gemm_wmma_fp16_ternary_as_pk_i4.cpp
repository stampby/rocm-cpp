...
bool run_gemm(const ProblemType& problem_size, const ExecutionConfig& config)
{
    ...
    // BitNet ternary: restrict weights to {-1, 0, +1}. Each byte holds 2 int4
    // lanes (upper/lower nibble). We iterate raw bytes and clamp each nibble
    // to one of three values, preserving the sign convention used by the
    // pk_i4 WMMA path (sign-extended int4 on lane).
    if(config.init_method >= 99)
    {
        auto* raw    = reinterpret_cast<int8_t*>(b_k_n.mData.data());
        size_t bytes = b_k_n.mData.size() * sizeof(BDataType);
        for(size_t i = 0; i < bytes; ++i)
        {
            auto tern = [&](uint8_t nibble) -> int8_t {
                uint8_t v = nibble & 0xF;
                // Deterministic 3-way fold on nibble bits:
                // 00-05 -> 0, 06-0A -> +1 (int4 0x1), 0B-0F -> -1 (int4 0xF)
                if(v < 0x6) return 0;
                if(v < 0xB) return 0x1;
                return 0xF;
            };
            ...
            int8_t  hi  = tern(b >> 4);
            int8_t  lo  = tern(b & 0xF);
            raw[i]      = static_cast<int8_t>(((hi & 0xF) << 4) | (lo & 0xF));
        }
        std::cout << "BitNet ternary (init_method>=99): B weights clamped to {-1,0,+1}"
                  << std::endl;
    }
